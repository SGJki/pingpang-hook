...
- `bin/pingpang-sound` — Hook 本体。按 `argv[2]` 分模式：`codex-approval`、`claude-approval`（黑名单判断 + 放行 JSON）、`approval`（仅响铃的遗留模式）、`complete`（完成音）。
...
4. 平台配置路径：Codex `~/.codex/hooks.json`；Claude Code `~/.claude/settings.json`；共享数据在 `~/.pingpang-hook/`（脚本 + `blacklist`）。
